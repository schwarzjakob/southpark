...
def get_map_data2(date):
    try:
        ...
        # Fetch parking lot capacity data
        query_parking_lots_capacity = f"""
        SELECT pl.id, pl.name AS name, pl.external AS external, plc.capacity AS capacity, dates.date
        FROM (SELECT generate_series('{start_date}', '{end_date}', '1 day'::interval)::date AS date) AS dates
        CROSS JOIN public.parking_lot pl
        LEFT JOIN public.parking_lot_capacity plc ON pl.id = plc.parking_lot_id AND plc.valid_from <= dates.date AND plc.valid_to >= dates.date AND plc.utilization_type = 'parking'
        ORDER BY pl.id, dates.date;
        """
        df_parking_lots_capacity = get_data(query_parking_lots_capacity)
        parking_lots_capacity = df_parking_lots_capacity.to_dict(orient="records")
        ...
        # Fetch parking lot occupancy data
        query_parking_lots_occupancy = f"""
        SELECT dates.date, pl.name AS parking_lot_name, COALESCE(SUM(pa.allocated_capacity), 0) AS occupancy
        FROM (SELECT generate_series('{start_date}', '{end_date}', '1 day'::interval)::date AS date) AS dates
        CROSS JOIN public.parking_lot pl
        LEFT JOIN public.parking_lot_allocation pa ON pa.date = dates.date AND pa.parking_lot_id = pl.id
        GROUP BY dates.date, pl.name
        ORDER BY dates.date, pl.name;
        """
        df_parking_lots_occupancy = get_data(query_parking_lots_occupancy)
        parking_lots_occupancy = df_parking_lots_occupancy.to_dict(orient="records")
        ...
        # Fetch parking lot allocation data
        query_parking_lots_allocations = f"""
        SELECT pa.parking_lot_id, pl.name AS parking_lot_name, pa.event_id, e.name AS event_name, e.color AS event_color, pa.allocated_capacity, pa.date
        FROM public.parking_lot_allocation pa
        JOIN public.parking_lot pl ON pa.parking_lot_id = pl.id
        JOIN public.event e ON pa.event_id = e.id
        WHERE pa.date BETWEEN '{start_date}' AND '{end_date}'
        ORDER BY pa.parking_lot_id, pa.event_id, pa.date;
        """
        df_parking_lots_allocations = get_data(query_parking_lots_allocations)
        parking_lots_allocations = df_parking_lots_allocations.to_dict(orient="records")
