...
# Some additional Endpoints included here, which are not included in the backend structure yet 
...
def add_event():
    try:
        ...
        halls = data["halls"]
        entrance = data["entrance"]
        previous_event_name = data.get("previous_event_name", None)
        color = data.get("color", None)

        # Initialize previous_event_id to None
        previous_event_id = None

        if previous_event_name:
            # Fetch the ID of the previous event
            query_previous_event = "SELECT id FROM event WHERE name = :previous_event_name"
            with engine.begin() as connection:
                result = connection.execute(text(query_previous_event), {"previous_event_name": previous_event_name})
                previous_event_row = result.fetchone()
                if previous_event_row:
                    previous_event_id = previous_event_row[0]
        
        # Insert into the event table
        query_event = """
        INSERT INTO event (name, early_assembly_start_date, early_assembly_end_date, assembly_start_date, assembly_end_date, runtime_start_date, runtime_end_date, disassembly_start_date, disassembly_end_date, late_disassembly_start_date, late_disassembly_end_date, color, previous_event)
        VALUES (:name, :early_assembly_start_date, :early_assembly_end_date, :assembly_start_date, :assembly_end_date, :runtime_start_date, :runtime_end_date, :disassembly_start_date, :disassembly_end_date, :late_disassembly_start_date, :late_disassembly_end_date, :color, :previous_event)
        RETURNING id
        """
        params_event = {
            "name": name,
            "early_assembly_start_date": dates.get("early_assembly", {}).get("start"),
            "early_assembly_end_date": dates.get("early_assembly", {}).get("end"),
            "assembly_start_date": dates["assembly"]["start"],
            "assembly_end_date": dates["assembly"]["end"],
            "runtime_start_date": dates["runtime"]["start"],
            "runtime_end_date": dates["runtime"]["end"],
            "disassembly_start_date": dates["disassembly"]["start"],
            "disassembly_end_date": dates["disassembly"]["end"],
            "late_disassembly_start_date": dates.get("late_disassembly", {}).get("start"),
            "late_disassembly_end_date": dates.get("late_disassembly", {}).get("end"),
            "color": color,
        ...
        with engine.begin() as connection:
            result = connection.execute(text(query_event), params_event)
            event_id = result.fetchone()[0]  # Fetch the newly generated event_id
            ...
            for hall_name in halls:
                ...
                for phase in ["early_assembly", "assembly", "runtime", "disassembly", "late_disassembly"]:
                    if phase in dates:
                        all_dates = calculate_date_range(
                            dates[phase]["start"], dates[phase]["end"]
                        )
            ...
            for entrance_name in entrance:
                entrance_id = connection.execute(
            ...
            all_queries = hall_occupation_queries + entrance_occupation_queries
...
#Add event: add demands
@app.route("/add_event_demands", methods=["POST"])
def add_event_demands():
    """
    Endpoint to add visitor demands to the database.
    """
    try:
        data = request.json
        ...
        demand_queries = []
        for event_date, demand_data in demands.items():
            query_demand = """
            ...
            params_demand = {
                "event_id": event_id,
                "date": event_date,
                "demand": demand_data["cars"] + 4 * demand_data["trucks"] + 3 * demand_data["buses"],
                "car_demand": demand_data["cars"],
                "truck_demand": demand_data["trucks"],
                "bus_demand": demand_data["buses"],
                "status": demand_data["phase"],
            }
            demand_queries.append((query_demand, params_demand))

        with engine.begin() as connection:
            for query, params in demand_queries:
                connection.execute(text(query), params)

        return jsonify({"message": "Demands added successfully"}), 200
    except Exception as e:
        logger.error("Failed to add demands", exc_info=True)
        return jsonify({"error": str(e)}), 500


#Add event: get visitors demands
@app.route('/visitor_demand', methods=['GET'])
def get_visitor_demand():
    """
    Endpoint to retrieve visitor demand data.
    """
    try:
        logger.info("Fetching visitor demand data from the database.")
        query = """
        SELECT 
            id,
            event_id,
        ...
        logger.error("Failed to fetch visitor demand data", exc_info=True)
        return jsonify({"error": str(e)}), 500



#Add Event: Post allocations the user confirmed
@app.route("/parking_lot_allocations", methods=["POST"])
def insert_parking_allocations():
    try:
        event_id = request.json.get("event_id")
        allocations = request.json["allocations"]
        queries = []

        for allocation_data in allocations:
            query_allocation = """
            INSERT INTO parking_lot_allocation (event_id, parking_lot_id, date, allocated_cars, allocated_trucks, allocated_buses)
            VALUES (:event_id, :parking_lot_id, :date, :allocated_cars, :allocated_trucks, :allocated_buses)
            """
            params_allocation = {
                "event_id": event_id,
                "parking_lot_id": allocation_data["parking_lot_id"],
                "date": allocation_data["date"],
                "allocated_cars": allocation_data["allocated_cars"],
                "allocated_trucks": allocation_data["allocated_trucks"],
                "allocated_buses": allocation_data["allocated_buses"]
            }
            queries.append((query_allocation, params_allocation))

        with engine.begin() as connection:
            for query, params in queries:
                connection.execute(text(query), params)

        return jsonify({"message": "Parking lot allocations inserted successfully"}), 201
    except Exception as e:
        app.logger.error("Failed to insert parking lot allocations", exc_info=True)
        return jsonify({"error": str(e)}), 500
    

#Add event / Dashboard: parking lot capacity & occupation on daily basis
@app.route('/daily_parking_lot_capacity_and occupation', methods=['GET'])
def get_parking_lot_utilization():
    """
    Endpoint to retrieve daily parking lot capacity & occupation data.
    """
    try:
        logger.info("Fetching daily parking lot capacity & occupation data from the database.")
        #SELECT * so the API endpoint does not need to be updated wehn the view changes
        query = """
        SELECT *
        FROM view_schema.view_daily_parking_lot_capacity_and_occupation
        ORDER BY date, parking_lot_name;
        """
        result = get_data(query)
        data = result.to_dict(orient="records")
        return jsonify(data), 200
    except Exception as e:
        logger.error("Failed to fetch daily parking lot capacity & occupation data", exc_info=True)
        return jsonify({"error": str(e)}), 500
