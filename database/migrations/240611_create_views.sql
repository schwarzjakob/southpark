...
SELECT DISTINCT
    ...
    CASE
        WHEN eo.entrance_id IS NOT NULL THEN
            CASE
                WHEN e.name = 'north' THEN pd.distance_north
                WHEN e.name = 'north_east' THEN pd.distance_north_east
                WHEN e.name = 'east' THEN pd.distance_east
                WHEN e.name = 'west' THEN pd.distance_west
                WHEN e.name = 'north_west' THEN pd.distance_north_west
                ELSE NULL
            END
        ELSE NULL
    END AS distance
...
JOIN
    public.visitor_demand vd ON pa.event_id = vd.event_id AND pa.date = vd.date
JOIN
    public.parking_lot pl ON pa.parking_lot_id = pl.id
JOIN
    public.hall_occupation ho ON e.id = ho.event_id AND pa.date = ho.date
JOIN
    public.hall h ON ho.hall_id = h.id
JOIN
    public.hall_parking_lot_distances pd ON ho.hall_id = pd.hall_id AND pa.parking_lot_id = pd.parking_lot_id
LEFT JOIN
...
WITH EventHalls AS (
    SELECT 
        ...
        vd.status, 
        CASE 
            WHEN en.name = 'north' THEN hp.distance_north 
            WHEN en.name = 'north_east' THEN hp.distance_north_east 
            WHEN en.name = 'east' THEN hp.distance_east 
            WHEN en.name = 'west' THEN hp.distance_west 
            WHEN en.name = 'north_west' THEN hp.distance_north_west 
            ELSE 0
        END AS distance 
    FROM 
        public.event e 
